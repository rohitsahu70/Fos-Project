...
def file_delete(request, file_id):
    if request.method == 'POST':
        file_to_delete = get_object_or_404(FileUpload, id=file_id, uploader=request.user)
...
def file_analyze(request, file_id):
    if request.method == 'POST':
        file_to_analyze = get_object_or_404(FileUpload, id=file_id)
        result = analyze_file(file_to_analyze.file.path)
        messages.success(request, "File analyzed successfully. Result: " + str(result))
        return redirect('file-manage')
    else:
        messages.error(request, "Invalid request")
        return redirect('file-manage')        
